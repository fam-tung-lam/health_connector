...
extension HealthDataTypeDto {
    ...
    func toHKSampleType() throws -> HKSampleType {
        ...
        case .heartbeatSeries:
            try HKSeriesType.make(from: HKDataTypeIdentifierHeartbeatSeries)
        ...
        case .insulinDelivery:
            try HKQuantityType.make(from: .insulinDelivery)
        ...
        case .peakExpiratoryFlowRate:
            try HKQuantityType.make(from: .peakExpiratoryFlowRate)
        ...
        case .irregularMenstrualCycleEvent:
            if #available(iOS 16.0, *) {
                try HKCategoryType.make(from: .irregularMenstrualCycles)
            } else {
                throw HealthConnectorError.unsupportedOperation(
                    message:
